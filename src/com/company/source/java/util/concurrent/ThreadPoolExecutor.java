...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     * 使用一个int来表示状态与线程数量，int 4字节，高三表示线程池状态，后29位用来记录个数
     *
     *
     ...
     * below).
     * 从 shuntdown状态 到 tidying 状态 需要注意
     *
     */
    ...
    // 存放worker数量以及线程池状态的
    // int 有32位， 高3位 充当线程池状态的标志位， 后29位 记录当前运行的worker数量
    ...
    // 线程池的5种状态 111  计算机用补码存储负数
    private static final int RUNNING    = -1 << COUNT_BITS;
    // 000  不接受新任务, 但处理排队任务
    private static final int SHUTDOWN   =  0 << COUNT_BITS;
    // 001  不接受新任务, 不处理排队任务, 并中断正在进行的任务
    private static final int STOP       =  1 << COUNT_BITS;
    // 010  所有任务都已终止, 工作线程为0, 线程转换到状态TIDYING, 将运行terminate()钩子方法
    private static final int TIDYING    =  2 << COUNT_BITS;
    // 011  标识terminate（）已经完成
    ...
    final void tryTerminate() {
        ...
        for (;;) {
            int c = ctl.get();
            // 这些条件不会进入Terminated状态
            // 1. 状态为 running
            // 2. 状态至少为 Tidying
            // 3. 状态为shutdown 并且 工作队列不为空
            // 4. 工作线程数不为0， 还有任务在执行
            if (isRunning(c) ||
            ...
            try {
                ...
                // cas 设置状态 为 tidying 并且 活跃线程数 为 0；
                if (ctl.compareAndSet(c, ctlOf(TIDYING, 0))) {
                    try {
                        // 设置成功后 调用terminated() 钩子方法
                        // 线程的方法
                        terminated();
                    } finally {
                        // 设置状态 为 terminated
                        ctl.set(ctlOf(TERMINATED, 0));
                        // 调用condition 的 signalAll() 接口唤醒所有等待线程
     ...
     * 检查是否可以根据当前池状态和给定边界（核心或最大值）添加新Worker
     * 如果是这样，则相应地调整Worker计数，并且如果可能，创建并启动一个新Worker，将 firstTask 作为其第一个任务运行。
     * 如果池已停止或有资格关闭，则此方法返回 false。如果线程工厂在询问时未能创建线程，它也会返回 false。如果线程创建失败，
     * 要么是由于线程工厂返回 null，要么是由于异常（通常是 Thread.start() 中的 OutOfMemoryError），我们干净地回滚。
     *
     ...
     * 当线程少于 corePoolSize 时（在这种情况下我们总是启动一个），或者当队列已满（在这种情况下我们必须绕过队列）时，
     * 使用初始第一个任务（在方法 execute() 中）创建工作线程.最初空闲线程通常是通过 prestartCoreThread 创建或替换其他垂死的工作线程。
     *
        ...
        for (;;) {
            ...
            if (rs >= SHUTDOWN &&
                ! (rs == SHUTDOWN &&
                   firstTask == null &&
                   ! workQueue.isEmpty()))
                // rs != running && (rs != shutdown || firstTask != null || workQueue.isEmpty()）
                // 1. 线程池状态不为Running 并且
                // 2. （1） 状态为stop，tidying，terminated 或 （2）提交的任务不为空 或（3）任务队列为空
                // 可以总结为：(1) 不为running且状态不为shutdown
                // （2）不为running 且 任务不为空
                // (3) 不为running 且 任务队列为空
                // 以上三种情况不启动任务
                return false;
            ...
            for (;;) {
                ...
                if (wc >= CAPACITY ||
                    ...
                    // 工作线程 >= 线程池容量 或 工作线程 >= 是否使用核心线程 ？ (核心线程数 或 最大线程数)
                    return false;
                // cas 工作线程数 + 1
                ...
                if (runStateOf(c) != rs)
                    // 线程池状态是否变化，如果变化 跳到外层循环重新获取线程池状态，否则内层循环重新cas
                    continue retry;
            ...
            if (t != null) {
                ...
                try {
                    ...
                    if (rs < SHUTDOWN ||
                        ...
                        // 状态为running 或 线程shutdown 但是还有任务需要执行
                        // 线程t是可运行的
                ...
                if (workerAdded) {
                    // 添加成功， 调用start方法启动线程。
                    t.start();   // 最终调用的是runWorker方法
                    workerStarted = true;
                }
            }
        } finally {
            if (! workerStarted)
                // 启动失败, 调用addWorkerFailed 回滚工作线程
                addWorkerFailed(w);
        }
     ...
     * 清理和记录 锤死的worker. 仅woeker线程调用，除非设置了 completedAbruptly，否则假定 workerCount 已经被调整以考虑退出。
     * 此方法从工作程序集中删除线程，
     * 并且如果由于用户任务异常而退出或如果少于 corePoolSize 工作人员正在运行或队列非空但没有工作人员，则可能终止池或替换工作人员。
     *
     * @param w the worker
    ...
    // DiscardOldestPolicy 调用poll丢弃最老的任务，执行当前任务
    // DiscardPolicy 抛弃任务
     ...
     * @param maximumPoolSize the maximum number of threads to allow in the
     *        pool
     *        最大线程数量，当阻塞队列慢了之后，如果还有任务提交，如果当前的线程数小于maximumPoolSize，则会新建线程来执行任务。
     *
     * @param keepAliveTime when the number of threads is greater than
     *        the core, this is the maximum time that excess idle threads
     *        will wait for new tasks before terminating.
     *        空闲线程的保活时间，如果某线程的空闲时间超过这个值都没有任务给它做，那么可以被关闭了。
     *        注意这个值并不会对所有线程起作用，如果线程池中的线程数少于等于核心线程数 corePoolSize，那么这些线程不会因为空闲太长时间而被关闭，
    ...
    // 所有任务的调度都是由execute方法完成的，这部分完成的工作是：检查现在线程池的运行状态、运行线程数、运行策略，决定接下来执行的流程，
    // 是直接申请线程执行，或是缓冲到队列中执行，亦或是直接拒绝该任务。其执行过程如下：
    ...
    public void execute(Runnable command) {
         ...
         * 如果运行的线程少于 corePoolSize，请尝试使用给定命令作为其第一个任务启动一个新线程。
         ...
         * 如果一个任务可以成功排队，那么我们仍然需要仔细检查我们是否应该添加一个线程（因为现有的线程自上次检查后就死了）或者池在进入此方法后关闭。
         ...
         */
        int c = ctl.get();
        // 线程数量小于 核心线程数, 创建并启动一个线程来执行新提交的任务
        ...
        // 走到这说明线程数量 大于等于 核心线程数
        // 线程池处理running状态，把提交的任务成功放入阻塞队列
        if (isRunning(c) && workQueue.offer(command)) {
            int recheck = ctl.get();
            // double check: 如果已经shutDown 回滚到入队前，remove()新添加的任务
            // 多线程环境下线程池的状态时刻在变化，ctl.get() 不是原子操作
            // 如果没有double check，线程处于非running状态，那么command可能永远不会执行。
            if (! isRunning(recheck) && remove(command))
                reject(command);
            // 这里已经放入阻塞队列，线程池处于running状态，但是没有线程
    ...
    public void shutdown() {
        ...
        try {
            ...
            //设置当前线程池状态为SHUTDOWN，如果已经至少是SHUTDOWN则直接返回
